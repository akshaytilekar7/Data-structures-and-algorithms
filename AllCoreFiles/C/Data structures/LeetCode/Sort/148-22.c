...
/*
TLE but works with 1 prev pointer
*/
...
void Partition(struct ListNode* head, struct ListNode* end,
	struct ListNode** partitionPrevNode)
{
...
void QuickSort(struct ListNode* head, struct ListNode* start, struct ListNode* end)
{
	if (IsInValidRange(head, start, end))
	{
		...
		Partition(start, end, &partitionPrevNode);
		...
		else if (partitionPrevNode->next != NULL && partitionPrevNode->next->next != NULL)
			QuickSort(head, partitionPrevNode->next->next, end);
	}
}
